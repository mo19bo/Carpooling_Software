...
public class ModifyFormActivity extends Activity {
	...
	private DriveRouteItem driveItem;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String jsonStr = intent.getStringExtra("curDriveItem");
		driveItem = DriveRouteItem.parseJsonObj(jsonStr);
		setContentView(R.layout.activity_modifyform);
		...
		//显示传入的其他路径信息(目前DatePicker无法手动设置年月日)
		spinner_personNum.setSelection(driveItem.getServer_num() - 1);
		String[] timeStr = driveItem.getStart_time().split(":");
		et_hour.setText(timeStr[0]);
		et_minute.setText(timeStr[1]);
		tv_price.setText(driveItem.getThetotalprice()+"");
		et_description.setText(driveItem.getBrief());
		
		cb_mon.setChecked(int2bool(driveItem.getWeeks1()));
		cb_tue.setChecked(int2bool(driveItem.getWeeks2()));
		cb_wed.setChecked(int2bool(driveItem.getWeeks3()));
		cb_thu.setChecked(int2bool(driveItem.getWeeks4()));
		cb_fri.setChecked(int2bool(driveItem.getWeeks5()));
		cb_sat.setChecked(int2bool(driveItem.getWeeks6()));
		cb_sun.setChecked(int2bool(driveItem.getWeeks7()));
	}
	...
	public void onClick_btn_publish(View view){
		...
		try {
			jsonObject.put("pid", driveItem.getPid());
			...
			new Thread(){
				...
				public void run() {
					...
					final String url = myStatus.getUrlString() + "modifydriverpublish.php";
					...
					try {
						...
						if("success".equals(result)){
							runOnUiThread(new Runnable(){
								@Override
								public void run() {
									Toast.makeText(ModifyFormActivity.this, "修改成功", 0).show();
							...
							runOnUiThread(new Runnable(){
								@Override
								public void run(){
									Toast.makeText(ModifyFormActivity.this, "修改失败", 0).show();
								}
							});
						}
					} catch (Exception e) {
	...
	private boolean int2bool(int integer){
		if(integer == 1){
			return true;
		}else{
			return false;
